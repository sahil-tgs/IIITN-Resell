...
const ProductCard = ({ product, isDarkMode }) => {
  return (
    <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1`}>
      ...
      <div className="p-5">
        ...
        <p className={`text-xl font-bold ${isDarkMode ? 'text-blue-400' : 'text-blue-600'} mb-3`}>
          ₹{product.price.toLocaleString()}
        </p>
        <div className="flex items-center gap-2 mb-4">
          <span className={`text-sm px-2 py-1 rounded-full ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
            {product.condition}
          </span>
          <span className={`text-sm px-2 py-1 rounded-full ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
            {product.location}
...
const MarketplacePage = ({ isDarkMode }) => {
  ...
  if (loading) return (
    <div className={`min-h-screen flex items-center justify-center ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-cream text-gray-900'}`}>
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-cream text-gray-900'} transition-colors duration-300`}>
      {/* Main Content */}
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="pt-8 pb-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className="text-4xl font-bold" style={{ fontFamily: 'Inter, sans-serif' }}>
                Marketplace
              </h1>
        ...
        {/* Search and Filters */}
        <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-6 rounded-2xl shadow-lg mb-8 transition-colors duration-300`}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="relative">
              <Tag className={`absolute left-3 top-1/2 transform -translate-y-1/2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`} size={20} />
              <select
        ...
        {filteredProducts.length === 0 ? (
          <div className={`text-center py-12 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl`}>
