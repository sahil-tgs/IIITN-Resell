...
export const ChatProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        // Socket.IO server lives on the chat-service's dedicated netty-socketio port,
        // NOT behind the HTTP API gateway. Override with REACT_APP_SOCKET_URL if needed.
        const socketUrl = process.env.REACT_APP_SOCKET_URL || "http://localhost:9092";
        console.log("Initializing socket connection to:", socketUrl);

        // Token is sent as a query param so the Java handler can read it via
        // HandshakeData#getSingleUrlParam("token"). Also kept under auth for
        // compatibility with the legacy Node server.
